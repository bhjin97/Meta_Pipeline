...
def send_slack_alert(context, status):
    ...
    if status == "success":
        emoji = "✅"
        title = "Airflow Load Test Task Success"
    else:
        emoji = "🚨"
        title = "Airflow Load Test Task Failed"
...
STREAMING_CONTAINER_NAME = "spark-streaming-load-test"
...
with DAG(
    dag_id="ecommerce_load_test_batch_pipeline",
    default_args=default_args,
    start_date=datetime(2026, 5, 7, tzinfo=KST),
    schedule=None,
    catchup=False,
    max_active_runs=1,
    tags=["ecommerce", "spark", "batch", "load-test"],
) as dag:
